...
def main():
    ...
    parser.add_argument("--local_rank",type=int,default=-1,help="distribute parallel parameter do not modify.")
    args = parser.parse_args()

    dist.init_process_group(backend="nccl")
    # 空闲的gpu:[3,4,5,6] 映射到 rank [0,1,2,3]
    device_ids = list(map(int,args.device_ids.split(',')))
    device = torch.device('cuda:{}'.format(device_ids[args.local_rank]))
    # print("device = ",device)
    # print("device_ids[args.local_rank]) = ",device_ids[args.local_rank])
